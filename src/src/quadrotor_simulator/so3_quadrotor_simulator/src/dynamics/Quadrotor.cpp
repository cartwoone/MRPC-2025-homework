                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  // v_dot = //?????
  v_dot = (R * Eigen::Vector3d(0, 0, thrust)) / mass_  // Thrust in world frame
        - Eigen::Vector3d(0, 0, g_)                 // Gravity
        + external_force_ / mass_                   // External force
        - resistance * vnorm / mass_;               // Air resistance
  ...
  // omega_dot = //??????
  omega_dot = J_.inverse() * (moments                          // Control moments
                           - cur_state.omega.cross(J_ * cur_state.omega)  // Coriolis term
                           + external_moment_);              // External moment
